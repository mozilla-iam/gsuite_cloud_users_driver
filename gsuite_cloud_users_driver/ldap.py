...
class User(object):
    ...
    def to_emails(self, users):
        ...
        for person in self.ldap_json:
            email = self._record_to_primary_email(person)
    ...
    def to_gsuite_account_structure(self):
        ...
        for person in self.ldap_json:
            try:
                ...
                if email.split("@")[1] in self.email_suffix_whitelist:
                    logger.debug(
                        "Adding user: {} to the list of potential accounts.".format(
                            person
                        )
                    )
                    users.append(
                        {
                            "primary_email": "{}@gcp.infra.mozilla.com".format(
                                email.split("@")[0]
                            )
                        }
                    )
    ...
    def _populate_membership(self):
        for group in self.groups:
            ...
            for user in self.users:
                ...
                if (
                    group_name
                    in self.users[user]["access_information"]["ldap"]["values"]
                ):
                    idx = self.groups.index(group)
                    self.groups[idx]["members"].append(
                        self._record_to_primary_email(self.users[user])
                    )

    def _record_to_primary_email(self, user):
        return "{}@gcp.infra.mozilla.com".format(
            user.get("primary_email")["value"].lower().split("@")[0]
        )
