...
def handle(event=None, context=None):
    ...
    for user in potential_gsuite_accounts:
        ...
        if email not in current_google_cloud_users and email not in user_whitelist:
            ...
            logger.debug(
                "Skipping user {} because they already exist in GCP.".format(user)
            )

    logger.info(
        "{} potential accounts, with {} existing GCP accounts".format(
            len(potential_gsuite_accounts), len(current_google_cloud_users)
        )
    )
    ...
    for email in current_google_cloud_users:
        if email not in ldap_user_emails and email not in user_whitelist:
            ...
            logger.debug(
                "Skipping user {} because this user exists in LDAP or is whitelisted.".format(
                    email
                )
            )

    logger.info(
        "Users collected the driver will create: {} and disable: {} users.".format(
            len(additions), len(disables)
        )
    )
    ...
    for user in additions:
        ...
        logger.info(
            "Account created or updated for: {}".format(user.get("primary_email"))
        )
